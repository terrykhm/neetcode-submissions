...
class Solution:
    ...
    def bucketSort(self, numsLength: int, frequencyDict: Dict) -> List[List[int]]:
        # When you multiply a list containing a mutable object (like an empty list [])
        # by a number, Python does not create a list of distinct, independent empty lists. Instead, it creates 
        #a list of references pointing to the exact same empty list in memory.
